...
from InstantViewer.views import home
from InstantViewer.models import Sequence
from InstantViewer.scr import cutToString

from InstantFinder.scr.OceanBlaster import query_to_file, BlastIt, parseOutBlastXml, complete
from InstantFinder.scr.OceanFinder import searchRefseq_by_accession, my_translate



class index(TemplateView):

    template_name = path.join("InstantFinder","index.html")
    _outBlast = []
    ...
    def inputFinder(self, inputSequenceForm):
        ...
        if blastn[0]: 
            ...
            outfileProt = open(path.join(settings.MEDIA_ROOT, 'InstantFinder', 'outfile.prot.fasta'),'w')
            outfileNucl = open(path.join(settings.MEDIA_ROOT, 'InstantFinder', 'outfile.nucl.fasta'),'w')
    ...
    def pickBlastRef(self, pickBlastRefForm, **kwargs):
        ...
        # export
        with open(path.join(settings.MEDIA_ROOT, 'InstantFinder', 'outfile.aligned.fasta'),'w') as handle:
